...
// EXTERNAL FUNCTIONS REQUIRED FROM FILE scheduler.c

extern int SimpleScheduler_main(int argc, char *argv[]);
extern void printSchedulerDetails();
...
char *readyQueue[MAX_READY_SIZE];
int ready_index = 0;                  // initializing the index to 0
...
void sys_call(int sig) {
    printf("\nInterrupt signal (SIGINT) received. Exiting shell and displaying the scheduled processes's details.\n\n");
    printSchedulerDetails();                         // prints the Scheduler Details of the Commands!
    exit(0);  
}
...
void loop_shell() {
    ...
    while (1) {
        ...
        // "start" is the signal to the scheduler to start running the processes. 
        // So this will call the scheduler & proceed from there.
        if (strcmp(command, "start") == 0) {
            printf("\nCalling SimpleScheduler...\n");
            char *scheduler_args[] = { "SimpleScheduler", NULL };  // Adjust args if needed
            SimpleScheduler_main(1, scheduler_args); 
            continue;
        } 
        ...
        // Identification of the "submit" command, which adds this command to the readyQueue
        if (strncmp(command, "submit", 6) == 0) {
            args = commandingArgs(command);

            if (ready_index < MAX_READY_SIZE) {
                if (ready_index < MAX_READY_SIZE) {
                    readyQueue[ready_index++] = strdup(args[1]);  // Add to readyQueue
                    printf("Job %s added to readyQueue\n", args[1]);
                }
            }
...
int main() {
    ...
    // Taking nCPU & tSlice as inputs from the user
    printf("Enter the number of CPUs (nCPU): ");
    scanf("%d", &nCPU);

    if (nCPU <= 0) {
        fprintf(stderr, "Error: NCPU must be a positive integer.\n"); // error handling for nCPU
        return 1;
    }

    printf("Enter the time slice (tSlice) in milliseconds: ");
    scanf("%d", &tSlice);

    if (tSlice <= 0) {
        fprintf(stderr, "Error: TSLICE must be a positive integer.\n"); // error handling for tSlice
        return 1;
    }

    printf("Starting shell with nCPU = %d and tSlice = %d ms\n", nCPU, tSlice);
